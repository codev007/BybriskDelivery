...
class PendingDetails extends StatefulWidget {
  ...
  PendingDetails(this.mPincode, {Key key}) : super(key: key);
  @override
...
class _PendingDetailsState extends State<PendingDetails> {
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Container(
              ...
              child: Text(deliveryPojo.baddress + ", " + deliveryPojo.bpincode),
            ),
            Container(
              child: Row(
                children: <Widget>[
  ...
  _fetchDeliveries() async {
    try {
      ...
      final response = await http.post(mApiController().pending,
  ...
  void _makeitPicked(String id, String orderID) async {
    try {
      String url = mApiController().pendingToPicked;
      Map<String, String> headers = {"Content-type": "application/json"};
      ...
      if (!body['error']) {
        sendAndRetrieveMessage(
            orderID, " Status updated : Picked successfully", token);
